...
ellnpts = 11   ## dont need
ellgrid = np.linspace(elllow, ellhigh, num = ellnpts)  ## dont need
"""
readVF = False
    ...
    for i1 in range(0, knpts): # over kt
        for i2 in range(0, znpts): # over zt, searching the value for the stochastic shock
            ...
            for i3 in range(0, knpts): # over k_t+1
                Y, w, r, T, c, i, u = Modeldefs(kgrid[i3], kgrid[i1], \   #not sure if that's right.
                zgrid[i2], params1)
                ...
                if temp > maxval:
                        ...
                        Vf1new[i1, i2] = temp
                        Pf1[i1, i2] = kgrid[i3]
                        Jf1[i1, i2] = zgrid[i2]    # not sure about this part
...
XYbar = np.array([kbar1, kbar1])   # not sure about this
X0 = np.array([kbar1])
Y0 = np.array([ellbar1])   # not sure about this
...
while (nconv):
    ...
    for i1 in range(0, knpts): # over kt
        for i2 in range(0, znpts): # over zt, searching the value for the stochastic shock
            maxval = -100000000000
            for i3 in range(0, knpts): # over k_t+1
                Y, w, r, T, c, i, u = Modeldefs(kgrid2[i3], kgrid2[i1], \  # not sure about this part
                zgrid[i2], params2)
                temp = u    # not sure about this part
                for i4 in range(0, znpts): # over z_t+1
                ...
                if temp > maxval:
                    ...
                    Vf2new[i1, i2] = temp
                    Pf2[i1, i2] = kgrid2[i3]
                    Jf2[i1, i2] = kgrid2[i4] # not sure about this part
