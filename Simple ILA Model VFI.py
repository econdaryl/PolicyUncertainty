...
print ('Converged after', count, 'iterations') 
print ('Policy function at (', (knpts-1.0)//2.0, ',', (znpts-1.0)//2.0, ') should be', \
kgrid[(knpts-1.0)//2.0], 'and is', PF1[(knpts-1.0)//2.0, (znpts-1.0)//2.0])
...
def PolSim(initial, nobs, ts, PF1, JF1, state1, params1, PF2, JF2, state2, \
    ...
    for t in range(0, ts-1):
        ...
        khist[t] = ((w[t]*ell[t]*(tau-1)+k[t+1]+c[t]-t[t])/
                    ((r[t]-delta)-(r[t]-delta)*tau+1)) 
        
    for t in range(ts-1, nobs):
        ellhist[t] = (w[t]/(chi*c[t]**sigma_z)*(1-tau))**(1/theta)
