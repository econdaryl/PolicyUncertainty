...
# unpack
    ...
    k4hist = np.zeros(nobs+1)
    l1hist = np.zeros(nobs)
    l2hist = np.zeros(nobs)
    l3hist = np.zeros(nobs)
    zhist = np.zeros(nobs)
    Khist = np.zeros(nobs)
    Lhist = np.zeros(nobs)
    ...
    k4fhist = np.zeros(nobs+2)
    l1fhist = np.zeros(nobs+1)
    l2fhist = np.zeros(nobs+1)
    l3fhist = np.zeros(nobs+1)
    ...
    for t in range(0, ts-1):
        ...
        Khist[t], Lhist[t], GDPhist[t], whist[t], rhist[t], T4hist[t], \
            Bhist[t], c1hist[t], c2hist[t], c3hist[t], c4hist[t], Chist[t], \
            Ihist[t], u1hist[t], u2hist[t], u3hist[t], u4hist[t] \
        ...
        if t < ts-2:  # use baseline model for predictions
            ...
            Kfhist[t+1], Lfhist[t+1], GDPfhist[t+1], wfhist[t+1], rfhist[t+1],\
                T4fhist[t+1], Bfhist[t+1], c1fhist[t+1], c2fhist[t+1], \
