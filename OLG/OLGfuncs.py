...
def Modeldefs(Xp, X, Y, Z, params):
    ...
    '''
    # unpack params
    [alpha, beta, gamma, delta, chi, theta, tau, rho, sigma, pi2, pi3, pi4, \
        f1, f2, f3, nx, ny, nz] = params
...
#        l3 = 0.0001
    ...
    C = c1 + pi2*c2 + pi3*c3 + pi4*c4
    I = GDP - C
    u1 = c1**(1-gamma)/(1-gamma) - chi*l1**(1+theta)/(1+theta)
    u2 = c2**(1-gamma)/(1-gamma) - chi*l2**(1+theta)/(1+theta)
    u3 = c3**(1-gamma)/(1-gamma) - chi*l3**(1+theta)/(1+theta)
    u4 = c4**(1-gamma)/(1-gamma)
    
    return K, L, GDP, w, r, T4, B, c1, c2, c3, c4, C, I, u1, u2, u3, u4
...
def Modeldyn(inmat, params):
    ...
    K, L, GDP, w, r, T4, B, c1, c2, c3, c4, C, I, u1, u2, u3, u4 = \
        Modeldefs(Xp, X, Y, Z, params)
    Kp, Lp, GDPp, wp, rp, T4p, Bp, c1p, c2p, c3p, c4p, Cp, Ip, u1p, u2p, \
        u3p, u4p = Modeldefs(Xpp, Xp, Yp, Zp, params)
    
    # Evaluate Euler equations
    El1 = (c1**(-gamma)*(1-tau)*w*f1) / (chi*l1**theta) - 1
    El2 = (c2**(-gamma)*(1-tau)*w*f2) / (chi*l2**theta) - 1
    El3 = (c3**(-gamma)*(1-tau)*w*f3) / (chi*l3**theta) - 1
    Ek2 = (c1**(-gamma)) / (beta*c2p**(-gamma)*(1 + rp - delta)) - 1
    Ek3 = (c2**(-gamma)) / (beta*c3p**(-gamma)*(1 + rp - delta)) - 1
    Ek4 = (c3**(-gamma)) / (beta*c4p**(-gamma)*(1 + rp - delta)) - 1
    
    return np.array([El1, El2, El3, Ek2, Ek3, Ek4])